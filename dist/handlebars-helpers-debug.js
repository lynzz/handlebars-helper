define("gallery/handlebars-helpers/1.3.0/handlebars-helpers-debug", [ "gallery/handlebars/1.0.2/handlebars-debug", "./dateformat-debug" ], function(require, exports, module) {
    (function(Handlebars, DateFormat) {
        ...
        if (hasDefine) {
            ...
            var DateFormat = require("./dateformat-debug");
        }
        var helpers = {
            // 时间格式
...
define("gallery/handlebars-helpers/1.3.0/dateformat-debug", [], function(require, exports, module) {
    ...
    DateFormat.ISO8601 = DateFormat.W3C = DateFormat.ATOM = DateFormat.JSON = "yyyy-MM-ddTHH:mm:sszzz";
    DateFormat.RFC822 = DateFormat.RFC1123 = DateFormat.RFC2822 = DateFormat.RSS = "ddd, dd MMM yyyy HH:mm:ss zzz";
    DateFormat.RFC850 = DateFormat.RFC1036 = DateFormat.COOKIE = "dddd, dd-MMM-yy HH:mm:ss zzz";
    DateFormat.CTIME = "ddd MMM d HH:mm:ss yyyy";
    DateFormat.DATE_TIME = "yyyy-MM-dd HH:mm:ss";
    ...
    DateFormat.prototype._charMaxLengthMap = {
        d: 4,
        f: 3,
        F: 3,
        g: 2,
    ...
    DateFormat.prototype._constructor = function(pattern, asUTC) {
        ...
        var charMaxLengthMap = this._charMaxLengthMap, len = pattern.length, pos = 0, buffer = "", chunks = [], quart = "", flush = function(type) {
            if (buffer !== "") {
    ...
    DateFormat.prototype.format = function(date) {
        ...
        var chunks = this._chunks, asUTC = this.asUTC, names = DateFormat.names[this.locale], pad = function(num, length) {
            num = num.toString();
        ...
        var patternValueMap = {
            ...
            y: yStr.substr(4),
            yy: yStr.substr(3),
            yyy: y < 1e3 ? yStr.substr(2) : y.toString(),
            yyyy: yStr.substr(1),
            yyyyy: yStr,
            z: zSign + zH.toString(),
            zz: zSign + zHStr,
    ...
    DateFormat.prototype.parse = function(str) {
        var chunks = this._chunks, asUTC = this.asUTC, names = DateFormat.names[this.locale];
        var patternParserMap = this._patternParserMap;
        patternParserMap.ddd = {
            type: "Array",
            value: names.day.abbr
        };
        patternParserMap.dddd = {
            type: "Array",
            value: names.day.full
        };
        patternParserMap.g = {
            type: "Array",
            value: names.era.abbr
        };
        patternParserMap.gg = {
            type: "Array",
            value: names.era.full
        };
        patternParserMap.MMM = {
            type: "Array",
            value: names.month.abbr
        };
        patternParserMap.MMMM = {
            type: "Array",
            value: names.month.full
        };
        patternParserMap.t = {
            type: "Array",
            value: names.amPm.abbr
        };
        patternParserMap.tt = {
            type: "Array",
            value: names.amPm.full
        };
        // extract keyword in relation to pattern
        var patternValueMap = {}, i, iLen, pos, posLen, chunk, chunkValue, parser, parserValue, matched, matchedValue, j, jLen;
        ...
        var now = new Date(), nowY = now.getUTCFullYear(), z, y, g, M, d, t, h, H, m, s, f;
        ...
        g = (g = patternValueMap.gg) ? names.era.full.indexOf(g) : (g = patternValueMap.g) ? names.era.abbr.indexOf(g) : 1;
        if (g === 0) {
            y *= -1;
        }
        M = (M = patternValueMap.MMMM) ? parseInt(names.month.full.indexOf(M), 10) : (M = patternValueMap.MMM) ? parseInt(names.month.abbr.indexOf(M), 10) : (M = patternValueMap.MM || patternValueMap.M) ? parseInt(M, 10) - 1 : 0;
        ...
        f = (f = patternValueMap.fff || patternValueMap.ff || patternValueMap.f || patternValueMap.FFF || patternValueMap.F || patternValueMap.F) ? parseInt(f, 10) : 0;
    ...
    DateFormat.prototype.toString = function() {
        var chunks = this._chunks, buffer = "", i, iLen, chunk;
        for (i = 0, iLen = chunks.length; i < iLen; i++) {
            chunk = chunks[i];
